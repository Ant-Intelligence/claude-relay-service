...
async function loadData() {
  ...
  try {
    const [pricingResp, statusResp] = await Promise.all([
      apiClient.get('/admin/models/pricing'),
      apiClient.get('/admin/models/pricing/status')
    ])
    if (pricingResp?.success) {
      pricingData.value = pricingResp.data || {}
    } else {
      showToast(pricingResp?.message || '加载模型价格失败', 'error')
    }
    if (statusResp?.success) {
      pricingStatus.value = statusResp.data || {}
    } else {
      showToast(statusResp?.message || '获取价格状态失败', 'error')
    }
  } catch (error) {
    showToast('加载模型价格失败: ' + (error?.message || '未知错误'), 'error')
  } finally {
    loading.value = false
  }
}
...
async function handleRefresh() {
  ...
  try {
    const result = await apiClient.post('/admin/models/pricing/refresh')
    if (result?.success) {
      showToast(result.message || '价格数据已刷新', 'success')
      await loadData()
    } else {
