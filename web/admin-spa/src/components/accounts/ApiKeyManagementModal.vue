  ...
  try {
    ...
    if (Array.isArray(account.apiKeys)) {
      ...
      try {
        ...
        logger.error('Failed to parse apiKeys:', error)
      }
    }
...
const resetApiKeyStatus = async (apiKey) => {
  ...
  try {
    ...
    logger.error('Failed to reset API key status:', error)
...
const deleteAllErrorKeys = async () => {
  ...
  try {
    ...
    logger.error('Failed to delete error API keys:', error)
...
const copyAllApiKeys = async () => {
  ...
  try {
    ...
    logger.error('Failed to copy all keys:', error)
